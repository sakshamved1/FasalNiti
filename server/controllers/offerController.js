...
const createOffer = async (req, res) => {
  try {
    ...
    if (!req.user) {
      return res.status(401).json({ success: false, message: 'Authentication required to place an offer' });
    }

    const buyerName = req.user.name;
    const buyerBusiness = (req.user.buyerDetails?.businessName) || req.user.name;
    const buyerId = req.user._id;
    const buyerTrustScore = (req.user.buyerDetails?.trustScore) || 90;
